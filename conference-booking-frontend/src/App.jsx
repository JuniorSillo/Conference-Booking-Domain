...
function App() {
  ...
  useEffect(() => {
    ...
    const loadBookings = async () => {
      ...
      try {
        const data = await fetchAllBookings({ signal, category })
        if (!signal.aborted) {
          setApiBookings(data)  // only replace API slice
          setStale([])
  ...
  }, [category, fetchTrigger])  // Deps: refetch on category or trigger change
  ...
  return (
    <>
      ...
      <main className="main-content">
        ...
        {loading && allBookings.length === 0 && staleBookings.length === 0 ? (
          <LoadingScreen />
        ) : error && allBookings.length === 0 && staleBookings.length === 0 ? (
          <ErrorScreen message={error} onRetry={triggerRefetch} />
        ) : (
          <BookingList
